...
// FIX: uso new RegExp() per evitare problemi di parsing del literal
// Rimossa class="thumb image-wrapper" (non esiste), CDN corretto da .cx a .tv
const ANIME_CARD_REGEX = new RegExp(
  '<a href="(https://www\\.hentaisaturn\\.tv/hentai/[^"]+)"[^>]*>\\s*<img src="(https://[^"]+)"[^>]*alt="([^"]+)"',
  'g'
);
...
async function extractDetails(url) {
  try {
    ...
    const html = await response.text();
    // FIX: uso indexOf invece di regex con [\s\S] che non e' supportato
    let description = "Nessuna descrizione disponibile.";
    const marker = 'id="shown-trama"';
    const markerPos = html.indexOf(marker);
    if (markerPos !== -1) {
      const openPos = html.indexOf('>', markerPos) + 1;
      const closePos = html.indexOf('</div>', openPos);
      if (openPos > 0 && closePos > openPos) {
        const raw = html.slice(openPos, closePos).replace(/<[^>]*>/g, '').trim();
